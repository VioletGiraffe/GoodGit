...
// A repository a scan found, the submodules it declares, and when it was last worked in, in the milliseconds
...
// the file everyday work rewrites. Submodules cost one `ls-files` per git repository, the same index the
...
// A repository whose query fails is answered without submodules, as one never opened is listed without them.
