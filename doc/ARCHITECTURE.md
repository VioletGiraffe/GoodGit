...
| | |
|---|---|
| `gitprocess` | `Git::run()` - async `QProcess` jobs, at most 4 concurrent, cancellable. `Git::runSync()` - blocking variant for the one startup moment before the event loop exists. Both apply the invariants below |
...
| `repository` | `Repository` - one repo: state, file entries, refresh, and every git action. The unit of the application |
