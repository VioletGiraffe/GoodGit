...
- `conventions.md`: code conventions specific to this codebase.
- `perf.md`: the `scripts/perf/` harnesses, what process-level performance they have established, and how to
  read their results.
...
| `hgcommandserver` | The hg transport: a pool of `hg serve --cmdserver pipe` processes, each a resident interpreter running one command per request, so interpreter startup is paid once per server rather than per call. Servers spawn on demand, bind to one repository and serve others via `-R`. A server keeps its bound folder in use, so it retires after 5 minutes idle, or 5 seconds after the last repository object containing that folder closes; the pool falls back to plain processes if hg cannot serve at all |
...
| `fileicons` | The system's icon for a file type, by name alone, so deleted and history-only files get one too. Cached per type as rendered pixmaps: Qt's file icon engines do not cache a missing file. macOS looks the type up natively (`fileicons_mac`), since Qt resolves icons there from the file on disk |
...
| `filelistview` | The file list widget both windows show: the column sizing and delegate they share, the sort proxy its clickable header drives - status rank then path, or path alone - and the path-copy actions both context menus offer. Answers and takes indexes in the source model's coordinates, so no window handles the shown order |
...
| `commitwindow` | One window = one repository. Owns a `Repository` and every user flow, and hosts the recent repositories dock. A submodule or untracked repository row opens another `CommitWindow` on that repository; its `historyChanged()` signal refreshes the parent. An unsent message survives the window closing and comes back on the next open while HEAD still points where it did. In its context menus an action blocked by the repository's state is shown disabled, one the selected rows can never reach is absent |
| `historywindow`, `historymodels` | The read-only commit history: log above; below it the selected commit's message body where it has one, over its file list beside a file's diff. Narrowed to one path it is a file history. Owns its own `Repository`, so a submodule's history opens without a `CommitWindow` on that submodule |
