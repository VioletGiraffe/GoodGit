...
command line - every operation is a subprocess; no libgit2, no reimplemented VCS logic. Git is the backend
that exists; the boundary a second one would arrive behind is described under Backends. Both window layouts
are drawn in `doc/UI/mockup.html`, generated by `doc/UI/mockup.gen.js`.
...
- **Line counts are optional per row and asked for separately.** `FileEntry::lineCounts` is an optional,
  ...
  answer nothing and cost the rows two columns and nothing else.
...
results call for. Check state survives a refresh, re-derived by path: persisting rows keep their state,
new rows default to checked unless untracked.
