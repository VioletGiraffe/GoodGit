...
| `changedfilesmodel` | The checkable file list; row styling comes from the theme as item data roles |
...
| `unifieddiff` | Classifies the lines of a unified diff, numbers each against both files, and pairs a removed line with the added one it was edited into - by similarity, never crossing, and only where the two are alike enough for the spans they differ in to be worth marking. Backend- and UI-free, so it can be tested directly |
| `textdiff` | Two lines aligned as sequences of tokens: how alike they are, and where they differ. Declines a pair too long or too unalike to be one edit |
| `difftextview` | The read-only view under the pane's header: added and removed lines banded to the full width and marked where a paired line differs, headers dimmed, and a gutter of old and new line numbers beside them. Diff colors come from the theme, the rest of its look from the stylesheet |
