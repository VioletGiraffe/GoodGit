...
(`git diff --name-status HEAD` plus `git ls-files --others`), and the checked rows are what the next commit
holds. The index is where that commit is assembled and nothing else: `git commit` takes the whole of it, so
the commit clears what is staged outside the checked paths, adds the checked ones, commits, and then puts
back what the clearing could have destroyed. It goes through the index rather than being limited to the
checked paths because a commit built from the working tree alone cannot carry a file mode: with
`core.filemode` off, which is git's default on Windows, nothing but the index records one. A repository with
no commits yet is not a mode of its own: the empty tree stands in for HEAD, and the same two queries and the
same commit carry the equivalence over unchanged. Mercurial answers the same shape with one `hg status` and
...
- What another tool staged outside the checked paths does not survive a commit: it is cleared, and only a
  file mode is put back, that being the one thing no working tree records. Every other entry's content is on
  disk throughout, so what is lost is the staging itself - and with it a hunk-by-hunk selection, which
  belongs to `git add -p` and has no expression here.
- Hunk/line staging is out: the row is the unit, and a checked row commits the whole file.
...
- A merge/cherry-pick/revert/rebase in progress is a separate mode: the index already holds that operation's
  result, conflict resolutions included, so the commit adds every tracked change to it rather than the
  checked rows alone, and discarding is refused - restoring a path to HEAD mid-operation would silently drop
  that operation's result for it.
...
Git's is `reset --soft`, not the usual `--mixed`, and that follows from the model above: the commit was
assembled in the index, so leaving the index alone puts the change back exactly as the commit took it.
Mercurial's is `uncommit`, not `rollback` - rollback undoes the last *transaction*, which after a pull is
the pull. It ships with hg but is off by default, so that one command switches it on for itself, and passes
`--allow-dirty-working-copy` because a partial commit leaves the rest of the list modified by design.
...
a temp file, never `-m`: it is the one argument with no bound on its length.
